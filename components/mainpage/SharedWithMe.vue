<template>
  <div class="my-project max-w-screen-2xl snap-x overflow-x-scroll">
    ...
    <section class="project-exist flex gap-3 scroll-ml-6 snap-start">
      <div
        v-for="project in getProjectSahred"
        :key="project.id"
...
export default {
  ...
  computed: {
    ...mapGetters('project', ['getProjectSahred'])
  },
  mounted () {
  ...
  methods: {
    ...
    updateProjectStatus () {
      this.noProject = this.getProjectSahred.length === 0
      this.projectExists = this.getProjectSahred.length > 0

      this.getProjectSahred.forEach((project) => {
