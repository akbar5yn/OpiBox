      ...
      <div class="flex items-center gap-11">
        ...
        <svg width="16" height="21" viewBox="0 0 16 21" fill="none" xmlns="http://www.w3.org/2000/svg"
          ...
          <path
            ...
            stroke="#19191B" stroke-width="2" stroke-linecap="round" />
        </svg>
        <button class="flex items-center px-6 py-3 gap-4 bg-[#6C61E1] text-white rounded-lg">
    ...
    <section>
      <div class="flex h-screen">
        ...
        <div class="p-9 px-20 mt-16 border-r w-1/2 relative">
          ...
          <div id="image-preview" class="h-full border-dashed border-[#95959D] flex flex-col items-center justify-center">
            <button v-if="currentImageIndex !== 0"
              class="slider-button absolute top-1/2 left-3 rounded-full shadow-xl p-4" @click="prevImage">
              <icon-galery-slider-icon class="rotate-180" />
            </button>

            <div ref="imageContainer" class="container-image h-fit relative">
              <div v-if="showMarker" :style="{ left: markerX + 'px', top: markerY + 'px' }"
                class="marker p-10 absolute transform -translate-x-1/2 -translate-y-1/2">
                <icon-galery-comment-mark class="cursor-move" />
              </div>
              <img v-if="Array.isArray(projectImage) && projectImage.length > 0" :src="projectImage[currentImageIndex]"
                alt="Image preview" class="object-contain w-full h-[100%]" />
              <img v-else :src="projectImage" alt="Image preview" class="object-contain w-full h-[100%]" />
            </div>
            <button v-if="Array.isArray(projectImage) && projectImage.length > 1"
              class="slider-button absolute top-1/2 right-3 rounded-full shadow-xl p-4" @click="nextImage">
        ...
        <div class="p-9 mt-16 border-r w-1/2">
          ...
          <div class="flex justify-between border-t-2 border-b-2 mt-5 py-5">
            <p class="font-cabinet-grotesk text-xl cursor-default" @click="selectedMenu = 'like'">
              {{ likeCount }}
              <span class="text-[#95959D]">Suka</span>
            </p>
            <p class="font-cabinet-grotesk text-xl cursor-default" @click="selectedMenu = 'comment'">
              {{ commentCount }}
          ...
          <div class="flex justify-between border-b-2 mt-5 py-5">
            <p v-if="isLiked" class="font-cabinet-grotesk text-xl flex items-center gap-2">
              <icon-galery-like-icon class="cursor-pointer" fill="red" stroke="red"
                @like-clicked="handleDissLikeProject" />
              <span class="text-[#95959D]">Suka</span>
            </p>
            <p v-else class="font-cabinet-grotesk text-xl flex items-center gap-2">
              <icon-galery-like-icon class="cursor-pointer" :fill="fillColor" :stroke="strokeColor"
                @like-clicked="handleLikeProject" />
              <span class="text-[#95959D]">Suka</span>
            </p>
            <!-- :to="`/comments/${getMyProject[currentImageIndex].id}`" -->
            <p class="font-cabinet-grotesk text-xl flex items-center gap-2 cursor-pointer"
          ...
          <div class="h-[65%] overflow-hidden overflow-y-scroll">
            ...
            <showComment v-if="selectedMenu == 'comment'" :show-marker="showMarker" @displayMarker="markerOnOff" />
          </div>
        </div>
      </div>
    </section>
